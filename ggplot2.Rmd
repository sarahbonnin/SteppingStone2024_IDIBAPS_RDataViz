...
Controlling the font size (axis text, title, legend, etc.) and style in your graph can be very important for the image readability.

Text size and style of ggplot2 graphs can be changed using the **theme()** function. While very powerful, it unfortunately utilizes a difficult structure.

We will illustrate how to make such changes on our first scatter plot.
...
**TIP**: package [**{ggsci}**](https://cran.r-project.org/web/packages/ggsci/vignettes/ggsci.html) contains a collection of pre-made color palettes inspired by colors used in scientific journals (and also by Science Fiction movies and series!).
